...
class InferSent(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        #print(sent.size())
        #print(action, sent.size(), sent_len)
        embedding = []
        ...
        for i in range(len(action_pos)):
            if flag ==0 and action_pos[0] == 0:
                ...
                temptattn = self.emb_att(temp)
                attns.append(temptattn.squeeze().data.cpu().numpy().tolist())
                emb_A = temptattn * temp
        ...
        tempattn2 = self.emb_att2(embedding) 
        embedding = tempattn2 * embedding
        ...
        return emb, attns, tempattn2.squeeze().data.cpu().numpy().tolist()
...
class actor(nn.Module):
    ...
    def get_gradient(self, h, x, s, reward, scope):
        if scope == "target":
            out = self.target_policy(h.cuda(), x.cuda(), s.cuda())
...
class critic(nn.Module):
    ...
    def forward(self, s1, s2, scope):
        # s1 : (s1, s1_len)
        if scope == "target":
            u, a, b = self.target_pred(s1)
            v, c, d = self.target_pred(s2)
            features = torch.cat((u, v, torch.abs(u-v), u*v), 1)     
            output = self.target_classifier(features)
            
        
        if scope == "active":
            u, a, b = self.active_pred(s1)
            v, c, d = self.active_pred(s2)
            features = torch.cat((u, v, torch.abs(u-v), u*v), 1)     
