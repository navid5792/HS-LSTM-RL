...
parser.add_argument("--actormodelname", type=str, default='model_actor_')
...
for i in range(1, 31):
    ...
    actorModel.load_state_dict(torch.load(os.path.join(params.outputdir, params.actormodelname + str(i) + ".pickle")))
    print("\nActor Loaded ", i)
    x = evaluate_RL(epoch, 0, 'test', final_eval = True)
    print(x)
    f.write(str(i) + "\t" + str(x) + "\n")
f.close()

'''
f = open("results_ai.txt", "w")
print("Both Critic and Actor Training......Done!")
actorModel.load_state_dict(torch.load(os.path.join(params.outputdir, params.actormodelname))) # params.actormodelname
print("\nActor Loaded")
for roots, dirs, files in os.walk("./savedir/ai"):
    pass
for file in sorted(files):
    if "test10_" in file:
        nli_net.load_state_dict(torch.load(os.path.join(params.outputdir, file))) # params.actormodelname
        print("\nCritic Loaded")
