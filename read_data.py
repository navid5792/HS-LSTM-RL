...
def get_MSRP_data():
    with open("dataset/data/MS_train.txt", "r") as f:
        data = f.readlines()
    with open("dataset/data/MSRP_actions1.txt", "r") as f:
        actions = f.readlines()
    train = dict()
    test = dict()
    valid = dict ()
    ...
    for i in range(1,4001):
        ...
        ac = actions[i].strip().split("\t")
        ac1_, ac2_ = [], []        
        for xx in ac[0].split():
            if xx == '0':
                ac1_.append(0)
            else:
                ac1_.append(1)
        
        for xx in ac[1].split():
            if xx == '0':
                ac2_.append(0)
            else:
                ac2_.append(1)
        s1_temp.append(" ".join(x[1].split()))
        s2_temp.append(" ".join(x[2].split()))
        a1_temp.append(ac1_)
        a2_temp.append(ac2_)
    ...
    train['s1'] = s1_temp
    train['s2'] = s2_temp
    train['a1'] = np.array(a1_temp)
    train['a2'] = np.array(a2_temp)
    ...
    test['s1'] = s1_temp
    test['s2'] = s2_temp
    test['a1'] = np.array(a1_temp)
    test['a2'] = np.array(a2_temp)
    test['label'] = np.array(label_temp)

    valid['s1'] = s1_temp
    valid['s2'] = s2_temp
    valid['a1'] = np.array(a1_temp)
    valid['a2'] = np.array(a2_temp)
...
def get_SICK_data():
    with open("dataset/data/SICK_train.txt", "r") as f:
        data = f.readlines()

    with open("dataset/data/SICK_actions_trigram.txt", "r") as f:
        actions = f.readlines()
...
def get_AI_data():
    with open("dataset/data/AI_train_.txt", "r") as f:
        data = f.readlines()
    with open("dataset/data/AI_actions_.txt", "r") as f:
        actions = f.readlines()
    train = dict()
    test = dict()
    valid = dict ()
