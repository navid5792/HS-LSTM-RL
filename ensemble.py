...
parser.add_argument("--outputdir", type=str, default='savedir/sicke/', help="Output directory")
parser.add_argument("--criticmodelname1", type=str, default='model_test61.pickle')
parser.add_argument("--criticmodelname", type=str, default='model_test61New10.pickle')
parser.add_argument("--actormodelname", type=str, default='model_actor61.pickle')
...
nli_net1 = critic(config_nli_model)
...
def evaluate_RL(epoch, typ = 0, eval_type='valid', final_eval=False):
    ...
    deleteCount = dict()
    wordCount = dict()
    w1 = np.arange(0.0, 1.0, 0.01) # with actor
    w2 = np.arange(1.0, 0.0, -0.01) # no actor
    o1, o2, targets = [], [], []
    for i in range(0, len(s1)):
        ...
        output1 = nli_net((left, left_len, actions_left, Rleft_action_pos), (right, right_len, actions_right, Rright_action_pos), scope = "target")

        output2 = nli_net1((left, left_len, left_action, left_action_pos), (right, right_len, right_action, right_action_pos), scope = "target")
        
        o1.append(output1.detach().cpu().numpy())
        o2.append(output2.detach().cpu().numpy())
        targets.append(int(tgt_batch))

    with open("ensemble.txt", "w") as f:
        correct = 0.
        for indexx in range(len(w1)):
            for i in range(len(o1)):
                output = list(w1[indexx] * o1[i][0] + w2[indexx] * o2[i][0])
            ...
            print(str(w1[indexx]) + "\t" + str(w2[indexx]) + "\t" + str(eval_acc) + "\n")
            f.write(str(w1[indexx]) + "\t" + str(w2[indexx]) + "\t" + str(eval_acc) + "\n")
            correct = 0.
...
nli_net1.load_state_dict(torch.load(os.path.join(params.outputdir, params.criticmodelname1)))
print("\nCritic Loaded")
